...
*   **Log Loss & The Casino:** First, we defined the probability function $g(p) = p^7(1-p)^3$. Then, we took the natural log of $g(p)$, leading to the log loss function $G(p) = \ln(g(p)) = 7\ln(p) + 3\ln(1-p)$. Setting the derivative to zero ($\frac{7}{p} - \frac{3}{1-p} = 0$) elegantly solves to $p = 0.7$.
